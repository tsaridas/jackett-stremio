...
addon.get('/stream/:type/:id.json', (req, res) => {
    ...
    const intervalId = setInterval(() => {
        ...
        if (!requestSent && ((elapsedTime >= config.responseTimeout) || (searchFinished && inProgressCount === 0 && asyncQueue.idle))) {
            ...
            console.log("Results : " + finalData.length + " / Timeout: " + (elapsedTime >= config.responseTimeout) + " / Finished Searching: " + searchFinished + " / Queue Idle: " + asyncQueue.idle() + " / Pending Downloads : " + inProgressCount + " / Discarted : " + (streams.length - finalData.length));
